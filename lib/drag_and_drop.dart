...
class _DragableWidgetState extends State<DragableWidget> {
  ...
  Future<DragItem?> provideDragItem(AsyncValueGetter<DragImage> snapshot, DragSession session) async {
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<DragItem?> textDragItem(
    ...
    final item = DragItem(image: await dragImage(), localData: 'text-item', suggestedName: 'PlainText.txt');
  ...
  Future<DragItem?> multipleRepresentationsDragItem(
    ...
    item.add(Formats.uri(NamedUri(Uri.parse('https://flutter.dev'), name: 'Flutter')));
    return item;
  }
    ...
    buildWidgetsForReaders(context, readers, (value) {
      setState(() {
        _content = ListView(
          ...
          children: value.intersperse(const SizedBox(height: 10)).toList(growable: false),
        );
      });
    });
    */
